...
import { Plus, Search, Send, Headphones, MessageSquare } from "lucide-react";
...
export default function Chats() {
  ...
  // Marquer le chat comme lu quand on le sélectionne et scroller vers le bas
  useEffect(() => {
          ...
          <div className="flex flex-1 flex-col h-full overflow-hidden border rounded-xl bg-background">
            ...
            <div className="border-b flex flex-row items-center justify-start p-3 space-x-3 bg-background z-10 h-16 shrink-0">
              <MrsAvatar
                ...
                className={cn(
                  "text-primary",
                  selectedChatData?.isSupport && "ring-2 ring-orange-500"
                )}
                size={40}
              ...
              <div className="flex flex-col">
                ...
                {selectedChatData?.isSupport && (
                  <p className="text-xs text-muted-foreground">
                    Support technique pour ce professionnel
                  </p>
                )}
              </div>
            </div>
